...
%DPLRR
alpha_dplrr_fileset = [10 10   0.1  10   1    1    10  0.1];
beta_dplrr_fileset =  [1  0.01 0.01 0.01 0.01 0.01 0.01 0.01];
%LRR
...
lambda_sslrr_fileset = [5 1 0.1 1 0.5 1 5 1];
%L-RPCA
w = 1;
...
for di = [2 4 5 6 7]
    ...
    X = data.X;
    [~,n] = size(X);
    fea = Normalize_test(X,'norm'); % for MATLAB 2017
    gnd = data.gnd;    
...
%   lambda_dplrr = 1/sqrt(log(n));
    lambda_dplrr = lambda_lrr_fileset(di);
